            ...
            {data ?
                <div className='wrapper-interest my-3 pb-3 bg-[#fff] h-auto'>
                    ...
                    <div className='grid gap-4 grid-cols-5 p-[16px]'>
                        {data.map((item, index) => {
                            return (
                                <Link to={{ pathname: '/postDetail', search: `?id=${item._id}` }} className="categories-item">
                                    <div key={item._id} className='interest-items block cursor-pointer hover:shadow-inner-lg hover:shadow-lg'>
